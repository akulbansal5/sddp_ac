...
function forward_pass(
    ...
    for i in 1:M
        ...
        for (depth, (node_index, noise)) in enumerate(scenario_path)
            ...
            println("       path: $(i), stage: $(depth), node: $(node_index), noise: $(noiseid), st_obj: $(subproblem_results.stage_objective), cost-to-go: $(costtogo[node_index][noise_id]), prob: $(scenario_paths_prob[i])")
        end
